...
class MergeClientsByDniRequest extends FormRequest
{
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            ...
            if ($keep->dni_normalized === null || $keep->dni_normalized === '') {
                $validator->errors()->add(
                    'keep_client_id',
                    'El cliente principal no tiene un DNI válido para unificar.'
                );

                return;
            }

            $mergeClients = Client::query()->whereIn('id', $mergeIds)->get(['id', 'dni_normalized']);
            foreach ($mergeClients as $client) {
                if ($client->dni_normalized !== $keep->dni_normalized) {
                    $validator->errors()->add(
                        'merge_client_ids',
                        'Todos los clientes a unificar deben compartir el mismo DNI.'
                    );
