...
<head>
    ...
    <style>
        ...
        .meta { font-size: 8px; color: #64748b; margin: 0 0 3px; }
        .legend { margin: 8px 0 4px; font-size: 8px; color: #475569; }
...
<body>
    ...
    @endif
    <p class="meta">Periodo: {{ $filters['start_date'] ?? '' }} al {{ $filters['end_date'] ?? '' }} · Generado el {{ $generatedAt ?? now()->format('d/m/Y H:i') }}</p>
    ...
    @php
        ...
        $barWidth = min(34, max(12, $slotWidth * 0.55));
        ...
        foreach ($chartRows as $index => $row) {
            ...
            $bars[] = [
                ...
                'width' => $barWidth,
                'height' => $barHeight,
                'label' => \Illuminate\Support\Str::limit($row['advisor_name'] ?? '', 14),
                'centerX' => $centerX,
            ];
    ...
    @if ($rowCount > 0)
        ...
        <svg width="100%" height="{{ $height }}" viewBox="0 0 {{ $width }} {{ $height }}" xmlns="http://www.w3.org/2000/svg">
            ...
            @foreach ($bars as $bar)
                ...
                <text x="{{ $bar['centerX'] }}" y="{{ $height - 18 }}" font-size="7" text-anchor="middle" fill="#475569">{{ $bar['label'] }}</text>
            @endforeach
        ...
        <tbody>
            @foreach ($chartRows as $index => $row)
                <tr>
                    ...
                    <td>{{ $row['team_name'] ?? '—' }}</td>
                    <td class="num">{{ number_format((float) ($row['sold_amount'] ?? 0), 2) }}</td>
